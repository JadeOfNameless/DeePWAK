...
test = map(x->x[1],tmp) |> gpu;
train = map(x->x[2],tmp) |> gpu;

X = train |> gpu;
Y = test |> gpu;

test,train = first(tmp) |> gpu
...
Θ = Parallel(vcat,map(ŋ->Chain(Dense(m => ŋ,relu),Dense(ŋ => m)),bfŋ)...)|>gpu
...
loss = (X,Y)->begin
    L = Flux.mse(Θ(X),Y)
    L_train = mapreduce(θ->Flux.mse(θ(train),train),vcat,Θ.layers)
    L_test = mapreduce(θ->Flux.mse(θ(test),test),vcat,Θ.layers)
    return L,hcat(L_train,L_test)
end
    
L,L_b = @showprogress map(1:100) do _
    map(loader) do l
        Flux.train!(loss,Flux.params(Θ),loader[1],opt)
    end
end

𝚯_s = map(Flux.state,𝚯);
jldsave("data/bfTheta.jld2";𝚯_s)

𝚯_s = JLD2.load("data/bfTheta.jld2", "𝚯_s");
Flux.loadmodel!(𝚯,gpu(𝚯_s))
